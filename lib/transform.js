...
function addAnchor(mode, header) {
  const pattern = /\s*<a\s+name\s*=\s*\".*\"\s*>.*<\/a>\s*/
  const hasLink = pattern.test(header.name);
  const link = (hasLink) ? header.name.replace(/\s*<a\s+name\s*=\s*\"/,'').replace(/\"\s*>.*/,'') : header.name;
  header.anchor = anchor(link, mode, header.instance);
  if(hasLink) {
    const body = header.name.replace(/\s*<a\s+name\s*=\s*\".*\"\s*>/,'').replace(/<\/a>\s*/,'');
    header.anchor = header.anchor.replace('['+ link +']', '[' + body + ']');
  }
  return header;
}
